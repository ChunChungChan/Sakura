...
void MySetupGlobalLight()
{
    ...
    globalPhongData.GlobalAmbientColor.Set(0.1, 0.1, 0.1, 1.0);
...
void MySetupLights()
{
    ...
    // First light (light #0).
    myLights[0].AmbientColor.Set(0.8, 0.6, 0.6,0.0f);    // Gray color
    myLights[0].DiffuseColor.Set(0.8,0.4,0.3,0.0f);      // Very light gray
    myLights[0].SpecularColor.Set(0.5, 0.4, 0.4,0.0f);   // Very light gray
    myLights[0].IsEnabled = true;                   // BE SURE TO ENABLE YOUR LIGHTS

    // Second light (light #1)
    myLights[1].AmbientColor.Set(0.9, 0.9, 0.9,0.0f);    // Yellow color
    myLights[1].DiffuseColor.Set(0.8, 0.8, 0.8,0.0f);    // Yellow
    myLights[1].SpecularColor.Set(0.7, 0.5, 0.5,0.0f);   // White
    myLights[1].IsEnabled = false;                     // BE SURE TO ENABLE YOUR LIGHTS

    // Third light (light #2)
    myLights[2].AmbientColor.Set(0.0, 0.4, 0.4,0.0f);    // Yellow color
    myLights[2].DiffuseColor.Set(0.3, 0.5, 0.5,0.0f);      // Very light gray
    myLights[2].SpecularColor.Set(0.6, 0.8, 0.8,0.0f);   // Very light gray
    myLights[2].IsEnabled = false;                     // BE SURE TO ENABLE YOUR LIGHTS

    myLights[3].DiffuseColor.Set(0.6, 0.6, 0.6,1.0f);      // Very light gray
...
void MySetupMaterials()
{
    ...
    myMaterials[0].AmbientColor.Set(0.2f, 0.45f, 0.2f, 0.0f);
    myMaterials[0].DiffuseColor.Set(0.4f, 0.9f, 0.4f,0.0f);
    myMaterials[0].SpecularColor.Set(1.0,1.0,1.0,0.0f);
