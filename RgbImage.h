...
public:
	RgbImage();
	RgbImage( const char* filename, bool flipImage = false );
	RgbImage( int numRows, int numCols );	        // Allocate a bitmap (uninitialized) of this size.
	~RgbImage();

	bool LoadBmpFile( const char *filename, bool flipImage = false);   // Loads the bitmap from the specified file
...
inline RgbImage::RgbImage( const char* filename, bool flipImage)
{
	NumRows = 0;
	NumCols = 0;
	ImagePtr = 0;
	ErrorCode = 0;
	LoadBmpFile( filename, flipImage );
}
