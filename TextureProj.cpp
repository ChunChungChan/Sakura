...
const double Xmax = 10.0;                // Control x dimensions of viewable scene
const double Ymax = 10.0;                // Control y dimensions of viewable scene
const double Zmin = -20.0, Zmax = 20.0;   // Control z dimensions of the viewable scene
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
	...
	case 'T':	testInfo = !testInfo;	// Test information on and off.
		return;
    case 'R':
...
void mouse_button_callback(GLFWwindow* window, int button, int action, int mods)
{
	if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS) {
		...
		lastViewAzimuth = viewAzimuth; viewDirection = lastViewDirection;
	}
